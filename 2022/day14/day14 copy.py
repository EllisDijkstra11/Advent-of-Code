...
def drop_sand(all_rocks):
    ...
    while not exit:
        ...
        sand_current = check_fall_order(sand_current, all_blocked, lowest)
...
def check_fall_order(sand_current, all_blocked, lowest):
    # Move sand down
...
def second(all_rocks):
    ...
    lowest_left, lowest_right = [500, lowest]
    for rock in all_rocks:
        if rock[1] == lowest and rock[0] < lowest_left:
            lowest_left = rock[0]
        if rock[1] == lowest and rock[0] > lowest_right:
            lowest_right = rock[0]
    
    lowest_left = [lowest_left - 1, lowest]
    lowest_right = [lowest_right + 1, lowest]

    all_sand = drop_sand(all_rocks)
    all_blocked = all_sand + all_rocks
    sand_start = [500, 0]

    # Add one grain to the bottom left and to the bottom right
    for _ in range(3):
        sand_current = check_fall_order(lowest_left, all_blocked, lowest)
        all_sand.append(sand_current)
        all_blocked.append(sand_current)
        sand_current = check_fall_order(lowest_right, all_blocked, lowest)
        all_sand.append(sand_current)
        all_blocked.append(sand_current)
    
    for y in range(lowest):
        print(lowest)
        found_sand = False
        for x in range(500 - y, 501, 1):
            if not [x, y] in all_blocked and not found_sand:
                all_sand.append([x, y])
            else:
                found_sand = True

        found_sand = False
        for x in range(500 + y, 499, -1):
            if not [x, y] in all_blocked and not found_sand:
                all_sand.append([x, y])  
            else:
                found_sand = True
